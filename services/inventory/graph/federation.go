...
func (ec *executionContext) resolveEntity(
	...
	case "ProductInventory":
		resolverName, err := entityResolverNameForProductInventory(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "ProductInventory": %w`, err)
		}
		switch resolverName {

		case "findProductInventoryByID":
			id0, err := ec.unmarshalNInt2int(ctx, rep["id"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findProductInventoryByID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindProductInventoryByID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "ProductInventory": %w`, err)
			}
...
func entityResolverNameForProductInventory(ctx context.Context, rep EntityRepresentation) (string, error) {
	for {
		var (
		...
		return "findProductInventoryByID", nil
	}
	return "", fmt.Errorf("%w for ProductInventory", ErrTypeNotFound)
}
